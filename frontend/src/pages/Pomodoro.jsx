...
export default function Pomodoro() {
        ...
        setMinutos((prevMin) => {
          ...
          if (modoAtual === 'foco') {
            setCiclosCompletos((ciclos) => {
              ...
              if (n % TOTAL_CICLOS === 0) {
                setMensagem(
                  `🎉 Você completou ${TOTAL_CICLOS} ciclos! Aproveite sua pausa longa 🌿`
                )
              } else {
                setMensagem(
                  '🍎 Foco concluído! Você ganhou maçãs! Hora de uma pausa curta ☕'
                )
              }
          ...
          if (modoAtual === 'pausaLonga') {
            setMensagem(
              '💪 Descansou bem? Vamos mais 4 ciclos 🍅'
            )
  ...
  const progresso =
    totalSegundosRef.current > 0
      ? restantes / totalSegundosRef.current
      : 1

  const offset =
    CIRCUNFERENCIA * (1 - progresso)
        ...
        {animacao && (
          <span
            style={{
              ...
              animation:
                'subirSumir 3s ease-out forwards',
